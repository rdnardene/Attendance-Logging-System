...
def contact():
    mess._show(title="Contact Me",message="If you find anything weird or you need any help ...'")


def about():
    mess._show(title="About",message="This Attendance System is designed by the student of SSCT")
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingImage\ " + name + "." + str(serial) + cs + "." + str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                bb = str(Id)
                dd = str(Id)
            cv2.putText(im, str(bb), str(dd), (x, y + h), font, 1, (0, 251, 255), 2)
    ...
    with open("Attendance\Attendance_" + date + ".csv", 'r') as csvFile1:
        ...
        for lines in reader1:
            ...
            if (i > 1):
                if (i % 2 != 0):
                    iidd = str(lines[0]) + '   '
                    tb.insert('', 0, text=iidd, values=(str(lines[2]), str(lines[4]), str(lines[6]), str(lines[8])))
    csvFile1.close()
    cam.release()
    cv2.destroyAllWindows()
...
window.subimg = ImageTk.PhotoImage(file="images/bgdashboard.jpg")
subimg = Label(window, bg="#26382b", image=window.subimg).place(x=0, y=0, width=1280, height=820)
...
frame1 = tkinter.Frame(window, bg="green")
...
#registretion frame
lbl = tkinter.Label(frame1, text="Enter ID",width=20  ,height=1  ,fg="black"  ,bg="green" ,font=('times', 17, ' bold ') )
lbl.place(x=0, y=35)
...
lbl2 = tkinter.Label(frame1, text="Enter Name",width=20  ,fg="black"  ,bg="green" ,font=('times', 17, ' bold '))
lbl2.place(x=0, y=90)
...
lbl3 = tkinter.Label(frame1, text="Enter Course & Section",width=20  ,fg="black"  ,bg="green" ,font=('times', 17, ' bold '))
...
message0=tkinter.Label(frame1,text="Follow the steps...",bg="green" ,fg="black"  ,width=39 ,height=1,font=('times', 16, ' bold '))
message0.place(x=7,y=275)

message1 = tkinter.Label(frame1, text="1)Take Images  ===> 2)Save Profile" ,bg="green" ,fg="black"  ,width=39 ,height=1, activebackground = "yellow" ,font=('times', 15, ' bold '))
...
#Attendance frame
lbl3 = tkinter.Label(frame2, text="Attendance Table",width=20  ,fg="black"  ,bg="green"  ,height=1 ,font=('times', 17, ' bold '))
...
clearButton = tkinter.Button(frame1, text="Clear", command=clear, fg="white", bg="green", width=11, activebackground = "white", font=('times', 12, ' bold '))
...
tb= ttk.Treeview(frame2,height =13,columns = ('name','course','date','time'),style="mystyle.Treeview")
tb.column('#0',width=52)
tb.column('name',width=130)
tb.column('course',width=130)
tb.column('date',width=130)
tb.column('time',width=130)
